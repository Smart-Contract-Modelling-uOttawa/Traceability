...
//	}	
	...
	def checkAssociation(Association association) {
		...
		var moduleName = association.getModuleType();
		var className = association.getClassType();
		var haveClass = (className != null) && (className.length() > 0);
		
		var f = (((((association.eContainer()) as Class).eContainer()) as Module).eContainer()) as Folder;
				...
				if (haveClass){
					for(Class c:m.getClasses()){
						if(c.getName().equals(className)){
							classFound = true;
							/* No break in Xtend, to continue the loop to finish */							
							//break;
						}
					}
				}
